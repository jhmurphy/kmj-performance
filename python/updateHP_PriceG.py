...
CSV_TO_READ = 'PriceG.csv'  # This assumes your part numbers are in the first column
...
for row in csv_Amazon:
    ...
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET PriceG = ? FROM Imaster INNER JOIN Plc ON Plc.PLCID = Imaster.PLCID WHERE PLC.PLC = ? AND Imaster.PartNumber = ?",ListPrice_float,PLC, PartNumber)
