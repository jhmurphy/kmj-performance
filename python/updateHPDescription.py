...
for row in csv_Amazon:
    ...
    print PLCID + ' ' + PartNumber + ' --> DISCONTINUED'   
    ...
    run_sql_statement(ODBC_CONN,"UPDATE Imaster SET Description = 'DISCONTINUED' FROM Imaster INNER JOIN ImasterECommerce ON Imaster.PartID = ImasterECommerce.PartID WHERE (OnHand = 0 OR OnHand IS NULL) AND ImasterECommerce.MapPrice = 0 AND PLCID = ? AND PartNumber = ?",PLCID, PartNumber)
